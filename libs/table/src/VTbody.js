...
var _default2 = {
  ...
  methods: {
    ...
    getComputedStyle(row, props) {
      return props === null ? props : _Table.default.isFunction(this.rowStyle) ? this.rowStyle.apply(this, [row, props.ariaRowIndex]) : this.rowStyle;
    },
    ...
    genColItemContext(row, item, dx, max) {
      return this.$createElement('td', {
        ...
        style: _Table.default.isFunction(item.cellStyle) ? item.cellStyle.apply(this, [row, item.field, dx]) : item.cellStyle,
        class: {
        ...
        item: row
      }) : this.genCellContext(item.formatter(row, item.field)), item.editable ? this.genEditContext(row, item.field, dx, max) : null]);
    },
